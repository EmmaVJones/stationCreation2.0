...
if(stationType %in% c('Riverine', 'Lacustrine')){
  WQS <- pin_get(paste0('ejones/', tolower(stationType)), board = 'rsconnect')
} else {
  WQS <- pin_get('ejones/estuarinePolys', board = 'rsconnect')
  WQSEL <- pin_get('ejones/estuarineLines', board = 'rsconnect')
}
